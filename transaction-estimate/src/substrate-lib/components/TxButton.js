...
function TxGroupButton(props) {
  const { api } = useSubstrate();
  async function estimateFee({ palletRpc, callable, inputParams }) {
    ...
    try {
      let resp = await fetch('/transaction/fee-estimate', {
        method: 'POST',
      ...
      if (resp.partialFee) {
        props.setFeeAmount(resp.partialFee);
      }
    } catch (error) {
  ...
  return (
    <>
      <button
        onClick={() => estimateFee(props.formState)}
